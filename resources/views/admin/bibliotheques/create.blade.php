<x-layouts.app>
    ...
    <script>
        ...
        document.getElementById('generate-image-btn').addEventListener('click', async function() {
            ...
            const btn = this;
            const suggestionsDiv = document.getElementById('image-suggestions');
            const previewDiv = document.getElementById('image-preview');

            btn.disabled = true;
            btn.innerHTML = '<i class="fas fa-spinner fa-spin mr-1"></i> AI analyse en cours...';

            try {
                const response = await fetch('{{ route('admin.bibliotheques.ai.image') }}', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
                ...
                if (data.success && data.suggestions) {
                    ...
                    previewDiv.querySelector('img').src = data.image;
                    document.getElementById('image-url-field').value = data.image;

                    // Show suggestions
                    suggestionsDiv.classList.remove('hidden');
                    suggestionsDiv.innerHTML = `
                        <p class="text-xs font-semibold text-slate-600 mb-2">📸 Plus d'options d'images:</p>
                        <div class="grid grid-cols-2 gap-2">
                            ${data.suggestions.map((img, index) => `
                                <img src="${img}" alt="Option ${index + 1}" 
                                     class="w-full h-24 object-cover rounded-lg cursor-pointer hover:opacity-75 transition-opacity suggestion-img"
                                     data-url="${img}">
                            `).join('')}
                        </div>
                    `;
                    ...
                    suggestionsDiv.querySelectorAll('.suggestion-img').forEach(img => {
                ...
                btn.disabled = false;
                btn.innerHTML = '<i class="fas fa-camera mr-1"></i> Générer Image AI';
            }
        });
